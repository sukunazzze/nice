...
from LogiMusic.misc import SUDOERS, db
from LogiMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from LogiMusic.utils.decorators.language import languageCB
from LogiMusic.utils.formatters import seconds_to_min
from LogiMusic.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from LogiMusic.utils.stream.autoclear import auto_clean
from LogiMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Logi.pause_stream(chat_id)
        ...
        await Logi.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Logi.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Logi.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Logi.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Logi.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
