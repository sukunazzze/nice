...
from LogiMusic.utils.database import (add_gban_user, get_served_chats, get_sudoers,
    ...
    if not message.reply_to_message:
        ...
        if user.id == from_user.id:
            ...
            m = await message.reply_text(
                f"**Logi Is Initializing Global Ban on {user.mention}**\n\nExpected Time : {len(served_chats)}"
            )
            number_of_chats = 0
    ...
    if user_id == from_user_id:
        ...
        if is_gbanned:
            ...
            for chat in chats:
                served_chats.append(int(chat["chat_id"]))
            m = await message.reply_text(
                f"**Logi Is Initializing Gobal Ban on {mention}**\n\nExpected Time : {len(served_chats)}"
            )
            number_of_chats = 0
            for sex in served_chats:
                try:
